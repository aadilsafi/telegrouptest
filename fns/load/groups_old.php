...
} else if ($show_public_groups & role(['permissions' => ['groups' => 'view_public_groups']])) {
...
$login_user_id = Registry::load('current_user')->id;
$columnsgg = $wheregg = null;

$columnsgg = ['site_role_id'];
$wheregg['user_id'] = $login_user_id;

$site_usergg = DB::connect()->select('site_users', $columnsgg, $wheregg);

if($site_usergg[0]['site_role_id'] == 5) {
    $where["AND"] = [
        "groups.secret_group" => "0"
    ];
}
...
foreach ($groups as $group) {
    ...
    if($site_usergg[0]['site_role_id'] != 5 || !empty($data["search"])) {
