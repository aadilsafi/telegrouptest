...
if ($force_request || role(['permissions' => ['groups' => 'create_groups']])) {
    ...
    $number_of_groups = DB::connect()->count("groups", ["created_by" => Registry::load('current_user')->id]);
    ...
    if($number_of_groups > 3){
        $result['success'] = false;
        $result['error_message'] = Registry::load('strings')->max_opener_group;
        $result['error_key'] = 'max_opener_group';
        $result['error_variables'] = [];
        $noerror = false;
    }
